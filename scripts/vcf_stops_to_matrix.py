...
def process_vcf(vcf, ignore_indel_over, gof):
    header = vcftools.VcfHeader(vcf)
                ...
                for sample in per_file_sample_list:
                    ...
                    if (is_passing
                        ...
                        if variant_type in ['INSERTION', 'DELETION', 'SNP']:
                            if variant_feature not in stops:
                                stops[variant_feature] = {}
                                gene_names.append(variant_feature)
                            if sample not in stops[variant_feature]:
                                stops[variant_feature][sample] = 1
                            else:
                                stops[variant_feature][sample] += 1
                            if variant_feature not in annotations:
                                annotations[variant_feature] = [variant_annot]
                            elif variant_annot not in annotations[variant_feature]:
                                annotations[variant_feature].append(variant_annot)
                        elif variant_type == 'uncalled_ambiguous':
                            if variant_feature not in amb_stops:
                                amb_stops[variant_feature] = {}
                            if sample not in amb_stops[variant_feature]:
...
def main(vcf_list, ignore_indel_over, gof, counts):
    with open(vcf_list, 'r') as fof:
        for vcf in vcf_list:
            vcf = vcf.rstrip()  # remove newline sign
            sys.stderr.write("Searching " + vcf)
            process_vcf(vcf, ignore_indel_over, gof)
    output_lof_matrix()


if __name__ == '__main__':
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument('vcf_list', help='file of VCF filenames, must be annotated with SNPEff', type=str)
    ...
    main(args.vcf_list, args.ignore_indel_over, args.GOF, args.counts)
